...
class FirstData(models.Model):
		...
		if not self.get_response:
			...
			for lines in response:
				...
				if lines.split(";")[11].replace('\"','') != '':
					...
					if self._get_code_response(details) == False:
						raise ValidationError("No existe el codigo %s" %details)
				else:
					details = "ACPT"
	...
	def _get_code_response(self,code):
		exist_code = self.env['first.data.code.response'].search([('name','=',code)]).id
		return exist_code
...
class FirstDataResponse(models.Model):
	...
	response_id = fields.Many2many("first.data.code.response")
